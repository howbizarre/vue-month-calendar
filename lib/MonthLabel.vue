...
watch(props, (newProps) => {
  resetDate.value = JSON.stringify(newProps) !== JSON.stringify(current);
  nameOfMonth.value = monthName(newProps.month)[0].toUpperCase() + monthName(newProps.month).slice(1);
});
</script>
...
<template>
  <div class="info-bottom">
    <div class="info-bottom-part">
      ...
      <button v-if="resetDate" @click="resetActiveDate" class="btn-month-base btn-full-rounded ml-2" aria-label="Reset to current date">
        today
      </button>
    </div>

    <div class="info-bottom-part">
      <button class="mr-3" @click="showMonthSettings" aria-label="Show month settings">
      ...
      <a href="https://github.com/howbizarre/vue-month-calendar" title="Vue 3 with Typescript Calendar and Tailwindcss">
